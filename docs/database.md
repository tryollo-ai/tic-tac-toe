...
- Schema: [`prisma/schema.prisma`](../prisma/schema.prisma) - three models: `Room`, `CompletedGame`, and `RoomViewer` (viewer-presence heartbeats for the live watcher count).
...
5. **Verify the tables exist.** Either open Prisma's GUI:
   ...
   You should see `rooms`, `completed_games`, and `room_viewers` (plus Prisma's
