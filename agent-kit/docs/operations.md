...
- **Token usage report.** A `Report token usage` step (`--mode usage`) appends a markdown table to the Summary tab showing total tokens, the input/output/cache split, turn count, avg tokens per turn, reported cost, and a per-model breakdown. The `result` event carries the authoritative run totals; if the run was killed or timed out before emitting one, the step falls back to summing per-turn usage from assistant events so a partial run still yields a useful breakdown. It is `continue-on-error` and runs on `if: always()`, so a usage hiccup never fails the job and failed/timed-out runs are still accounted for.
